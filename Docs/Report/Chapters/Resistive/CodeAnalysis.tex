...
Code allows to run application with Timer 1 or 2, depending on TIMER C macro definition value. It also allows to configure if interruption mechanism used to trigger application when capacitor is discharged by defining CCP1 C macro or not. If CCP1 is defined, Capture Compare PWM (CCP\nomenclature{CCP}{Capture Compare PWM}) capture mode is used instead of INT0 interruption.
